...
function [CTE, closestWaypoint, headingError, xV, yV]  = fnCalculateProjectedPathError(currentPose, AIW_Data, previewDistance)

    xV_Current = currentPose(1);
    yV_Current = currentPose(2);
    thetaV = currentPose(3);

    % Project position forward (previewDistance * heading)
    xV = xV_Current + previewDistance * cos(thetaV);
    yV = yV_Current + previewDistance * sin(thetaV);
